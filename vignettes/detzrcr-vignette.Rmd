...
The released version of the package can be installed from [CRAN](https://cran.r-project.org/web/packages/detzrcr/index.html)
...
(Fig. \ref{fig1}) in the *shiny* interface. 

![Data input tab\label{fig1}](data_input.png)
...
dat <- detzrcr::check_conc(dat)
knitr::kable(head(dat, 10), format='pandoc', caption='Example input format')
```
...
Data is uploaded in the `Data input` tab (Fig. \ref{fig1}). From this tab a concordance
filter can be applied to the data (defaults to $\pm$ 10%). The different features
of the R-package can then be used by selecting the appropriate panel tab in the
user interface. By selecting the `Density distribution` tab the user can
plot the U-Pb age data as KDEs (user selectable fixed bandwidth) or PDPs
(local variable bandwidth based on the absolute $1\sigma$ uncertainty). Which
samples to plot and in what order can be selected from the `Select sample` menu
(Fig. \ref{fig2}). This feature is also available for all other plot and table options.

![Density distribution tab\label{fig2}](kde.png)
...
![UQ vs. LQ with mixture model\label{fig3}](uqlq.png)
...
where the likeness (Satkoski et al. 2013) and 1-O (Andersen et al. 2016)
...
and Hf data in the lower triangle (Fig. \ref{fig4}). All tables can be saved to
csv-files.

![1-O table\label{fig4}](1-o.png)

The 1-O table can also be displayed graphically (Fig. \ref{fig5}) where red indicates
$1-O \geq 0.05$, white $0 < 1-O < 0.05$ and green $1-O = 0$.

![1-O table as figure\label{fig5}](oplot.pdf)
...
the aformentioned decay constant and CHUR values)
* $^{176}$Lu/$^{177}$Hf value used in two step depleted model age $=0.015$

### References
Andersen, T., Elburg, M., Cawthorn-Blazeby, A., 2016. U–Pb and Lu–Hf zircon data
in young sediments reflect sedimentary recycling in eastern South Africa. J.
Geol. Soc. London. 173, 337–351. doi:10.1144/jgs2015-006
...
48–57. doi:10.1016/j.epsl.2008.06.010
...
Ann. Math. Stat. 27, 642–669. doi:10.1214/aoms/1177728174
...
data in time and space. GSA Bull. 125, 1783–1799. doi:10.1130/B30888.1
